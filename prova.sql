...
-- 1 --
INSERT INTO Livros (titulo, autor, ano_publicacao, disponivel, categoria, isbn, editora, quantidade_paginas, idioma) VALUES
('As Crônicas de Nárnia', 'C.S. Lewis','1970', TRUE, 'Fantasia' , '978-0064471190','HaperCollins','768', 'Francês');

 -- 2 --
 UPDATE Livros, ano_publicacao
 SET disponivel = FALSE
 WHERE ano_publicacao < 1980;
 
 -- 3 --
 UPDATE Livros, editora
 SET editora 
 WHERE '1984' AS 'Plume Books';
 -- Alias --
 
 -- 4 --
 DELETE idioma LIKE ('Inglês')
 FROM Livros 
...
WHERE categoria
GROUP BY  Livros
ORDER BY count (livros);
 
 
 -- 7 --
SELECT (titulo, autor, ano_publicacao, disponivel, categoria, isbn, editora, quantidade_paginas, idioma)
FROM livros
ORDER BY (titulo, autor, ano_publicacao, disponivel, categoria, isbn, editora, quantidade_paginas, idioma) DESC;
 
 -- 8 --
 
SELECT AVG(quantidade_paginas) FROM Livros WHERE disponivel = TRUE;
...
-- 10 --
SELECT * FROM Livros
WHERE titulo lIKE ('S%') AND 1975 < ano_publicacao > 1990;
